...
import { ArrowLeft, Send, CheckCircle2, Paintbrush, Globe, Box, MessageSquare } from "lucide-react";
...
const STATUS_MAP: Record<string, { label: string; className: string }> = {
  submitted:    { label: "Brief Submitted",   className: "bg-blue-500/15   text-blue-400 border-blue-500/20"   },
  reviewing:    { label: "Under Review",      className: "bg-violet-500/15 text-violet-400 border-violet-500/20" },
  quoted:       { label: "Quote Ready",       className: "bg-amber-500/15  text-amber-500 border-amber-500/20"  },
  accepted:     { label: "Quote Accepted",    className: "bg-cyan-500/15   text-cyan-400 border-cyan-500/20"   },
  in_progress:  { label: "In Progress",       className: "bg-primary/15    text-primary border-primary/20"    },
  review_ready: { label: "Ready for Review",  className: "bg-emerald-500/15 text-emerald-500 border-emerald-500/20" },
  ...
  if (req === null) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-muted-foreground mb-6">This customization request doesn't exist or you don't have access.</p>
        <Button asChild className="rounded-xl"><Link href="/dashboard/customizations">Go Back</Link></Button>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8 flex flex-col min-h-[calc(100vh-4rem)]">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold mb-2 tracking-tight">Custom Build Request</h1>
          ...
          <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
            ...
            <div className="p-5 flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="font-semibold text-sm truncate">{idea?.title ?? "Unknown Product"}</p>
                <Link href={`/ideas/${idea?.slug}`} className="text-xs text-primary hover:underline mt-1 inline-block">
                  View Idea Page →
                </Link>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6 flex flex-col-reverse">
            {!req.messages || req.messages.length === 0 ? (
              <div className="text-center py-10 my-auto">
                <p className="text-muted-foreground text-sm">No messages yet. Send a message to start chatting with the team.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={m._id} className={cn("flex flex-col max-w-[85%]", isAdmin ? "mr-auto" : "ml-auto items-end")}>
                      ...
                      <div className="flex items-center gap-2 mt-1.5 text-[10px] text-muted-foreground">
                        <span className="font-medium">{isAdmin ? "Admin Team" : "You"}</span>
                        <span>•</span>
